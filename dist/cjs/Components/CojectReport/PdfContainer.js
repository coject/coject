...
// Pdf Components
const PdfBody_1 = __importDefault(require("./PdfComponents/PdfBody"));
const PdfHeader_1 = __importDefault(require("./PdfComponents/PdfHeader"));
const PdfFooter_1 = __importDefault(require("./PdfComponents/PdfFooter"));
const ProcessItem_1 = require("./PdfComponents/ProcessItem");
// Add Fonts
...
function estimateRowHeight(row, columns, json, fontSizeCm = 0.4, lineHeightMultiplier = 1.2) {
    ...
    for (const key in columns) {
        if (columns[key]?.hide)
            continue;
        if ((0, ProcessItem_1.isImageValue)(row[key], columns[key])) {
...
const PdfContainer = ({ data, jsonData, parameter, reportName }) => {
    if (!jsonData || !jsonData.Body || !jsonData.Body.items) {
        return (react_1.default.createElement(renderer_1.Document, { title: reportName },
            react_1.default.createElement(renderer_1.Page, { size: "A4", style: { fontFamily: 'Almarai' } },
                react_1.default.createElement(PdfHeader_1.default, { apiData: data, headerData: jsonData, tableData: data, pageIndex: 0, parameter: parameter, totalPages: 1 }),
                react_1.default.createElement(PdfBody_1.default, { apiData: data, bodyData: jsonData, tableData: {}, pageIndex: 0, parameter: parameter, totalPages: 1 }),
                react_1.default.createElement(PdfFooter_1.default, { apiData: data, footerData: jsonData, tableData: data, pageIndex: 0, parameter: parameter, totalPages: 1 }))));
    }
    const pages = computeSequentialLayout(jsonData, data);
    const maxLength = pages.length;
    return (react_1.default.createElement(renderer_1.Document, { title: reportName }, pages.map((pageLayout, pageIndex) => {
        return (react_1.default.createElement(renderer_1.Page, { key: pageIndex, size: jsonData?.PaperSize, orientation: jsonData?.PageOrientation, style: {
                fontFamily: 'Almarai',
            ...
            react_1.default.createElement(PdfBody_1.default, { apiData: data, bodyData: jsonData, tableData: pageLayout.tableData, pageIndex: pageIndex, parameter: parameter, totalPages: maxLength || 1, adjustedLayouts: pageLayout.adjustedLayouts }),
