...
export const Request = async ({ dataSource, mode, data, apiUrlId, dispatch, callback }) => {
    ...
    if (Method.toLowerCase() === "get" || Method.toLowerCase() === "delete")
        await RequestCreation[Method.toLowerCase()](`${dataSource?.baseUrl ? dataSource?.baseUrl : process.env.VITE_URL}${APIUrl || ""}${APIUrlId ? "/" + APIUrlId : ""}`, { "headers": Headers })
            .then((Response) => SuccessAction(Response))
            .catch((Error) => CatchAction(Error));
    else
        await RequestCreation[Method.toLowerCase()](`${dataSource?.baseUrl ? dataSource?.baseUrl : process.env.VITE_URL}${APIUrl || ""}${APIUrlId ? "/" + APIUrlId : ""}`, Data, { "headers": Headers })
            .then((Response) => SuccessAction(Response))
            .catch((Error) => CatchAction(Error));
};
