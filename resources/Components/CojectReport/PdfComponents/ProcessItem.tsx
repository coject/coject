...
const ensureWrap = (val: any): string => {
...
export const isImageValue = (val: any, colDef?: any): boolean => {
    ...
    if (/^https?:\/\/.*\.(png|jpg|jpeg|gif|webp|svg)(\?.*)?$/i.test(str)) return true;
...
export const getImageSource = (val: any): string => {
...
const ProcessItemCommonStyles = (item: any, json: any): any => {
    ...
    if (item?.fontStyle) {
        ...
        if (stylesList.includes('font-bold')) styles.fontWeight = 'bold';
        if (stylesList.includes('font-italic')) styles.fontStyle = 'italic';
        if (stylesList.includes('font-underline')) styles.textDecoration = 'underline';
    }
...
const ProcessTextObjectItem = (item: any, json: any): any => {
...
const ProcessDatabaseFieldItem = (item: any, json: any): any => {
    return ProcessItemCommonStyles(item, json);
};

const ProcessPictureItem = (item: any, json: any): any => {
    const x = getDecimal(item?.x);
    const y = getDecimal(item?.y);
    const width = getDecimal(item?.width);
    const height = getDecimal(item?.height);
...
const ProcessSpecialFieldItem = (item: any, json: any): any => {
    return ProcessItemCommonStyles(item, json);
}

const ProcessTableObjectItem = (item: any, json: any): any => {
    const x = getDecimal(item?.x);
    const y = getDecimal(item?.y);
    const width = getDecimal(item?.width);
...
const ProcessQrCodeObjectItem = (item: any, json: any): any => {
    const x = getDecimal(item?.x);
    const y = getDecimal(item?.y);
    const width = getDecimal(item?.width);
    const height = getDecimal(item?.height);
...
const QrCodeGeneratedImage = ({ item, styles }: { item: any; styles: any }) => {
        ...
        try {
            ...
            if (buffer) {
                return new Promise((resolve) => {
                    const reader = new FileReader();
                    reader.onloadend = () => resolve((reader.result as string) || "");
                    reader.readAsDataURL(buffer as any);
                });
            }
        } catch (error) {
    ...
    return (
        <View style={{ ...styles }}>
            {item?.data ? (
                <Image src={generateQrCode() as any} style={{ width: '100%', height: '100%', objectFit: 'contain' }} cache={false} />
            ) : null}
        </View>
    );
};

const processTableColumns = (item: any, json: any): any => {
    ...
    visibleKeys.forEach((key) => {
        ...
        if (col?.fontSize) styleDict.fontSize = `${col.fontSize}pt`;
        if (col?.color) styleDict.color = col.color;
        if (col?.backgroundColor) styleDict.backgroundColor = col.backgroundColor;
        if (col?.textAlign) styleDict.textAlign = col.textAlign;
...
const getSpecialFieldValue = (key: string, context: any): string | number => {
    switch (key) {
...
const formatDate = (date: any, format: string = "dd/MM/yyyy HH:mm", dir?: string): string => {
    ...
    const map: Record<string, any> = {
        dddd: days[lang as 'ar' | 'en'][date.getDay()],
        dd: pad(date.getDate()),
        d: date.getDate(),
        MMMM: months[lang as 'ar' | 'en'][date.getMonth()],
        MMM: months[lang as 'ar' | 'en'][date.getMonth()].slice(0, 3),
...
const ProcessPanelItem = (item: any, json: any): any => {
    return ProcessItemCommonStyles(item, json);
};
    ...
    if (item?.type === "text-object") {
        ...
        return (
            <View style={{ ...itemStyles }}>
                {source ? <Image src={source} cache={false} /> : null}
            </View>
        )
    }
        ...
        const hasMerge = updatedColumnsEntries.some(([_, col]: [string, any]) => col.merge?.colSpan > 1);
        const headerItems: any[] = [];
        for (let i = 0; i < updatedColumnsEntries.length; i++) {
            const [key, col]: [string, any] = updatedColumnsEntries[i] as [string, any];
        ...
        tableRows.forEach((row: any, rowIndex: number) => {
            if (rowIndex > 0) {
                footerConfigs.forEach((footer: any, fIdx: number) => {
            ...
            groupConfigs.forEach((group: any, groupIndex: number) => {
                ...
                if (groupChanged || rowIndex === 0 || val !== lastGroupValues[group.field]) {
                    ...
                    processedRows.push({
                        type: 'group-header',
                        text: (group.headerText || "").replace(/\{(\w+)\}/g, (_match: string, key: string) => row[key] ?? ""),
        ...
        const renderTableHeader = (headerKey: string) => (
            <View key={headerKey} style={{ flexDirection: json?.Direction == 'rtl' ? 'row-reverse' : 'row' }}>
                {headerItems.map((hItem: any, idx: number) => {
                    ...
                    if (hItem.type === 'single') {
                        return (
                            <View key={idx} style={{
                                ...
                                <Text wrap={true} style={{ textAlign: hItem.col.Styles.textAlign || 'left', width: '100%', hyphens: 'none' } as any}>{ensureWrap(hItem.col.text || hItem.key)}</Text>
                            </View>
                        );
                    } else {
                        let totalWidthCm = 0;
                        hItem.subCols.forEach(([_, sc]: [any, any]) => totalWidthCm += parseFloat(sc.Styles.width));
                        return (
                            <View key={idx} style={{ flexDirection: 'column', width: `${totalWidthCm}cm` }}>
                                <View style={{
                                    ...hItem.col.Styles,
                                    ...
                                    <Text wrap={true} style={{ textAlign: 'center', hyphens: 'none' } as any}>{ensureWrap(hItem.col.colSpanLabel || "")}</Text>
                                </View>
                                <View style={{ flexDirection: json?.Direction == 'rtl' ? 'row-reverse' : 'row' }}>
                                    {hItem.subCols.map(([sk, sc]: [string, any], sidx: number) => {
                                        ...
                                        return (
                                            <View key={sidx} style={{
                                                ...
                                                <Text wrap={true} style={{ textAlign: sc.Styles.textAlign || 'left', width: '100%', hyphens: 'none' } as any}>{ensureWrap(sc.text || sk)}</Text>
                                            </View>
                                        );
                                    })}
                                </View>
        ...
        const renderTableFooters = (rows?: any[], specificFooterIndex?: number) => {

            if (!item?.footer || !Array.isArray(item.footer)) return null;

            const footersToRender = specificFooterIndex !== undefined
                ? [item.footer[specificFooterIndex]]
                : item.footer;

            return footersToRender.map((footerRow: any, fIdx: number) => {
                ...
                if (rows === undefined && groupCol) return null;
                if (rows !== undefined && !groupCol) return null;
                ...
                return (
                    <View key={`footer-${actualIdx}`} style={{ flexDirection: json?.Direction == 'rtl' ? 'row-reverse' : 'row' }} wrap={false}>
                        {colKeys.map((colKey: string, cIdx: number) => {
                            ...
                            if (fCol.valueSource === 'staticValue') {
                                ...
                                if (fCol.aggregate === 'sum') {
                                    cellValue = values.reduce((a: any, b: any) => a + b, 0);
                                } else if (fCol.aggregate === 'count') {
                                    cellValue = values.length;
                                } else if (fCol.aggregate === 'avg') {
                                    cellValue = values.length ? values.reduce((a: any, b: any) => a + b, 0) / values.length : 0;
                            ...
                            if (fCol.fontSize) cellStyle.fontSize = `${fCol.fontSize}pt`;
                            else cellStyle.fontSize = '12pt';

                            if (fCol.color) cellStyle.color = fCol.color;
                            if (fCol.backgroundColor) cellStyle.backgroundColor = fCol.backgroundColor;
                            if (fCol.textAlign) cellStyle.textAlign = fCol.textAlign;
                            ...
                            if (isImage) {
                                ...
                                return (
                                    <View key={cIdx} style={{ ...cellStyle, alignItems: align, justifyContent: 'center' }}>
                                        {imgSrc ? (
                                            <Image src={imgSrc} cache={false}
                                                style={{ height: imgHeight, maxHeight: '100%', maxWidth: '100%', objectFit: 'contain', alignSelf: align }}
                                            />
                                        ) : null}
                                    </View>
                                );
                            ...
                            return (
                                <View key={cIdx} style={{ ...cellStyle }}>
                                    <Text style={{ textAlign: cellStyle.textAlign || 'left', width: '100%', hyphens: 'none' } as any}>{ensureWrap(cellValue)}</Text>
                                </View>
                            );
                        })}
                    </View>
        ...
        return (
            (isTableVisibleOnThisPage && (tableRows.length || (pageIndex ?? 0) == 0)) ? (
                <View style={{ ...itemStyles, borderTop: '1px solid #000' }} wrap={true}>
                    ...
                    <View style={{ flexDirection: 'column' }}>
                        {processedRows.map((pRow: any) => {
                            if (pRow.type === 'group-header') {
                                return (
                                    <View key={pRow.key} style={{
                                        ...
                                        <Text style={{ fontWeight: 'bold', fontSize: '10pt', hyphens: 'none' } as any}>{ensureWrap(pRow.text)}</Text>
                                    </View>
                                );
                            }
                            ...
                            return (
                                <View key={pRow.key} style={{ flexDirection: json?.Direction == 'rtl' ? 'row-reverse' : 'row' }} wrap={false}>
                                    {updatedColumnsEntries.map(([key, col]: [string, any], i: number) => {
                                        const { color, backgroundColor, fontWeight, ...filteredStyles } = col?.Styles || {};
                                        let cellValue;
                                        if (col?.isCustom && col?.valueSource) {
                                            switch (col.valueSource) {
                                                case 'rowNum': cellValue = idx + 1; break;
                                                case 'staticValue': cellValue = col?.staticValue ?? ''; break;
                                                case 'parameter': cellValue = parameter?.[col?.parameter] ?? ''; break;
                                        ...
                                        if (isImage) {
                                            ...
                                            return (
                                                <View key={i} style={{ ...filteredStyles, backgroundColor: backgroundColor, alignItems: align, justifyContent: 'center' }}>
                                                    {imgSrc ? (
                                                        <Image
                                                            src={imgSrc}
                                                            cache={false}
        ...
        return <QrCodeGeneratedImage item={item} styles={itemStyles} />;
    }
    else if (item?.type === "panel") {
        const itemStyles = ProcessPanelItem(item, json);
        return (
            <View style={{ ...itemStyles }}>
                {item?.body?.items?.map((childItem: any, index: number) => (
                    <ProcessItem key={index} apiData={apiData} item={childItem} json={json} pageIndex={pageIndex} tableData={tableData} parameter={parameter} totalPages={totalPages} adjustedLayouts={adjustedLayouts} />
                ))}
            </View>
        );
    }
