...
export const Form: FC<iForm> = ({ name, offline, mode, getForm, storeData, customData, rowIndex, schema, dataSource, onSubmit, onSubmitData, onSubmitClear, gridMode, children, dispatch, storeSchema, ...props }) => {
    ...
    const onFormSubmit = ( SubmitData: any ) => {
        ...
        if ( dataSource ) {
            if ( !offline ) {
                ...
                Request({ dataSource, mode, data: ((dataSource && dataSource.formData) ? DataForm : DataJSON), apiUrlId: (dataSource.primaryKey ? Data[dataSource.primaryKey] : Data.id), callBack: () => ResetFunction(), dispatch }).then();
            } else {
                    ...
                    <div className="row">
                        { schema && !!schema?.length && schema.map((Field: any, Index: number) => {
                            ...
                            switch (Field.type?.toLowerCase()) {
                                case "input":
                                    return Field.actionTemplate ? <React.Fragment key={Index}>{Field.actionTemplate(Field)}</React.Fragment> : <div key={Index} className={Field.media ? Field.media : "col-12"}><Input name={Field.field} {...Field.props} value={Data[Field.field] ? Data[Field.field] : Field?.props?.value} /></div>
                                case "select":
                                    return Field.actionTemplate ? <React.Fragment key={Index}>{Field.actionTemplate(Field)}</React.Fragment> : <div key={Index} className={Field.media ? Field.media : "col-12"}><Select name={Field.field} {...Field.props} value={Data[Field.field] ? Data[Field.field] : Field?.props?.value} /></div>
                                case "datePicker":
                                    return Field.actionTemplate ? <React.Fragment key={Index}>{Field.actionTemplate(Field)}</React.Fragment> : <div key={Index} className={Field.media ? Field.media : "col-12"}><DatePicker name={Field.field} {...Field.props} value={Data[Field.field] ? Data[Field.field] : Field?.props?.value} /></div>
                                default:
                                    return null;
                            }
                        })}
                        { children }
                        { schema && !!schema?.length && Fields.includes("button") ? schema.map((Field: any, Index: number) => {
                            if (Field.type === "button") {
                                if (Field.template) {
                                    return <React.Fragment key={Index}>{Field.template(Methods.getValues(), (storeSchema && !!Object.keys(storeSchema).length && storeSchema.loading))}</React.Fragment>
                                } else {
                                    return <div key={Index} className={Field.media ? Field.media : "col-12"}><button {...Field.props} disabled={storeSchema && !!Object.keys(storeSchema).length && storeSchema.loading}>{Field.field}</button></div>
                                }
                            } else return null;
                        }) : ( !(offline && !mode) && <button type="submit" disabled={storeSchema && !!Object.keys(storeSchema).length && storeSchema.loading}>Save</button> )}
                    </div>
                </form>
            </FormProvider>
        </React.Fragment>
