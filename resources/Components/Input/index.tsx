...
export const Input: FC<iInput> = ({ name, value, helperText, validation, required, onChange, ...props }) => {
    ...
    useEffect(() => {
        ...
        const Arabic: boolean = !!validation?.arabic && !!inputValue && !(/^[\u0621-\u064A\u064B-\u0652\u0660-\u0669 0-9'"“”._\/\-()\[\]{}\u060C\u061B\u061F\u06D4:\u2013\u200C\u200D%\$\s]+$/ui.test(`${inputValue}`));
        const English: boolean = !!validation?.english && !!inputValue && !(/^[A-Za-z0-9\s.,'"()\/:;!?`\u2013\u2014\-%\$]+$/ui.test(`${inputValue}`));
