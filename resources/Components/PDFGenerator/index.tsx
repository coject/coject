...
export const PDFGenerator: FC<any> = ({button, landscape, fileName, title, children}) => {
    ...
    const generatePDF = async () => {
        ...
        html2canvas(element).then((canvas) => {
            ...
            for (let i = 0; i < totalPages; i++) {
                ...
                const ctx: any = croppedContent.getContext("2d");
    ...
    return (
        <Fragment>
            <Box className={`${classes.root} coject_pdf`}>
                <Button variant={'contained'} onClick={() => setModal(true)}>{button ?? 'PDF Report'}</Button>
                <Modal open={modal} setOpen={setModal} title={title ?? 'PDF Report'} className={classes.modal}>
                    { pdfPreview ?
                        <Fragment>
                            <iframe src={pdfPreview} className={classes.preview} />
                            <Button className={classes.button} variant={'contained'} href={pdfPreview} download={`${fileName ? (fileName + ".pdf") : "example.pdf"}`}>Download PDF</Button>
                        </Fragment> :
                        <Fragment>
                            ...
                            <Button className={classes.button} variant={'contained'} component={'a'} onClick={generatePDF}>Preview PDF</Button>
                        </Fragment>
                    }
                </Modal>
            </Box>
